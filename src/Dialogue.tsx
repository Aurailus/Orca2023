...
interface Props {
	...
	setMessagesHook: (hook: (messages: string[]) => void) => void;
}
...
export default function DialogManager(props: Props) {
	...
	props.setMessagesHook(setMessages);
	...
	const currentMessage = messages[messageInd];
	...
	}, [ currentMessage ]);Math.min(messageInd + 1, messages.length)
	...
	function handleClick() {
		...
		if (currentLetter >= plainTextMessage.length) {
			if (messageInd > messages.length - 1) return;

			setCurrentLetter(0);
			let nextInd = Math.min(messageInd + 1, messages.length);
			setMessageInd(nextInd);
			if (nextInd >= messages.length) setTimeout(() => props.onClose?.(), 800);
			setAnimation(nextInd >= messages.length ? 'animate-message-out' : 'animate-text-next');
		}
		else {
	...
	useEffect(() => {
		const spaceCheck = (e: KeyboardEvent) => {
			if (e.key === ' ') handleClick();
		};

		window.addEventListener('keydown', spaceCheck);
		return () => window.removeEventListener('keydown', spaceCheck);
	})
	...
	const questionsVisible = messageInd === messages.length - 1 && currentLetter >= plainTextMessage.length;

	function handleClickQuestion(key: string) {
		if (messageInd !== messages.length - 1) return;
		props.onClose?.(key);
		setMessageInd(messages.length);
		setAnimation('animate-message-out');
	}

	return (
		<div class='fixed inset-0 w-full h-full z-[1000] cursor-pointer'
			onClick={handleClick}
		>
			<div
				class='absolute left-1/2 bottom-12 pb-12 -translate-x-1/2 w-max h-max font-handwritten text-[36px]'
			>
				<div
					class={classes(
						'relative w-[800px] bg-white bg-clip-content font-handwritten transition-all duration-200 pointer-events-none p-6',
						animation
					)}
					style={{
						...
						borderWidth: 16,
						height: `calc(${Math.max(chatHeight + 64, 120)}px)`
					}}
				>
